...
\section{Connect}
The \mil{Connect} first clears the text areas, then parses the server information from the \textsc{gui} and connects to the corresponding socket. When the \textsc{tcp/ip} connection is established, it calls \mil{startCommunication} which is described in section \ref{sec:com}. Finally it sets the title of the editor.
...
them to the lower pane. See the left part of figure \ref{fig:event}.
