...
\renewcommand*{\textex}[1]{\ensuremath{\text{\texttt{#1}}}}
\renewcommand*{\opname}[1]{\ensuremath{\text{\textit{#1}}}}
...
is no delay between user input and its effects on the screen. However,
the implementation becomes a little more difficult.
...
text. If the other parties edit their versions of
the text in the meantime, they have to transform the received operation
so that it does what was intended.

Example: A and B start with text \textex{abcdefg}. A inserts
\textex{123} at offset 2 and gets \textex{ab123cdefg}, then sends the
operation to B. Before B receives it, she removes two characters at
offset 4 and gets \textex{abcdg}, then she sends this operation to A.
Now, A and B receive the operations from each other, but if they applied
them directly, they would end up with different versions of the text: A
with \textex{ab12defg} and B with \textex{ab123cdg}. In this case, the
result B gets already appears as a sensible combination of the two
operations. So only the remove operation A received has to be shifted by
three characters to the right, so that it ends up with that text, too.
Mathematically:
...
only consistent, but not sensible, results.
